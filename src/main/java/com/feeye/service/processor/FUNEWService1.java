...
public class FUNEWService1 {
    ...
    private String getFlightInfo(CloseableHttpClient httpClient, RequestConfig config, HttpClientContext context, String serviceId,
                                 String flightNo, boolean isVerify, String verifyCode, String cookie) throws IOException {
...
//        String depdate = param.getDeparuteDate();
        ...
        RequestConfig IndexConfig = RequestConfig.custom().setSocketTimeout(timeout)
                .setConnectTimeout(timeout).setConnectionRequestTimeout(timeout)
                .setExpectContinueEnabled(false)
                .setStaleConnectionCheckEnabled(true)
                .build();
        ...
        logger.info("FU进入首页cookie:" + indexCookie);
...
//            String desc = FingerPrintUtil.getDesc();
            ...
            //设置请求头
            post.setHeader("Proxy-Authorization", "Basic " + Base64.encodeBase64String("feeyeapp:feeye789".getBytes(StandardCharsets.UTF_8)));
            ...
            System.out.println("FU查询航班返回：" + back);
            if (backJson.size() == 3) {
                JSONArray segmentsArray = backJson.getJSONObject("data").getJSONArray("segments");
                if (segmentsArray.isEmpty()) {
                    System.out.println("FU没有查询到相关航班信息" + back);
                } else {
                    System.out.println("FU查询到的航班信息" + back);
                ...
                if ("10000".equals(backJson.get("sta").toString())) {
                    ...
                    if (cookie.contains("ci")) {
                        ...
                        config = RequestConfig.custom()
                                ...
                                .setStaleConnectionCheckEnabled(true)
                                .build();
                        return getFlightInfo(httpClient, config, context, serviceId, flightNo, false, "", "");
                    }
                    ...
                    System.out.println("FU识别查询航班验证码:" + result);

                    return getFlightInfo(httpClient, config, context, serviceId, flightNo, true, result, cookie);
                    ...
                    config = RequestConfig.custom()
                            ...
                            .build();
                    //验证码输入正确,回调不带验证码参数
                    return getFlightInfo(httpClient, config, context, serviceId, flightNo, false, "", cookie);
...
//        }
        ...
        FUNEWService1 funewService = new FUNEWService1();
        CloseableHttpClient httpclient = null;
        ...
        System.setProperty("https.protocols", "TLSv1,TLSv1.1,TLSv1.2,SSLv3");
        httpclient = builder.build();
        funewService.getFlightInfo(httpclient, config, context, "", "", false, "", "");
    }
}