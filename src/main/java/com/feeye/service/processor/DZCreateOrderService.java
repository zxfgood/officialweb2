...
//	         String res = (String) inv2.invokeFunction("ConvertSpell", "LUO");
...
//        String jsFileName="C:\\�½��ļ���\\plugins.min.js";
//        FileReader reader = new FileReader(jsFileName);   // ִ��ָ���ű�   
//        engine.eval(reader);   
//        if(engine instanceof Invocable) {    
//        Invocable invoke = (Invocable)engine;    // ����merge��������������������    
//        String a = (String)invoke.invokeFunction("ConvertSpell", "��"); //������js��aa����
//        System.out.println(a);   
//        String c = (String)invoke.invokeFunction("getPhoneNum", "12345678"); //������js��aa����
...
//        }
    ...
    public String login(CloseableHttpClient httpclient, RequestConfig config, BasicCookieStore cookieStore,
        ...
        try {
            get = new HttpGet("http://new.hnair.com/hainanair/ibe/common/login.do");
            get.setConfig(config);
            target = new HttpHost("new.hnair.com", 443, "https");
            get.setHeader("Referer", "http://new.hnair.com/hainanair/ibe/common/login.do");
            get.setHeader("Host", "new.hnair.com");
            ...
            // ��ȡ��֤���һ������
            get = new HttpGet(
                    "http://new.hnair.com/hainanair/ibe/profile/clickIamgeCode.do?methodName=ImageClick&verkey=LOGIN&timestampFalg="
                            + random + "");
            get.setConfig(config);
            target = new HttpHost("new.hnair.com", 443, "https");
            get.setHeader("Referer", "http://new.hnair.com/hainanair/ibe/common/login.do");
            ...
            String imagisRight = jo.getString("imagisRight");
            Map<String, String> map = new HashMap<String, String>();
            if (StringUtils.isNotEmpty(imagisRight) && imagisRight.contains("|")) {
                String[] str = imagisRight.split("\\|");
            ...
            for (int i = 0; i < 5; i++) {
                // ��ȡ��֤��ڶ������� ��һ����
                String r = "0." + (long) ((Math.random() + 1) * 10000000000000000L);
                get = new HttpGet(
                        "http://new.hnair.com/hainanair/ibe/profile/clickIamgeCode.do?methodName=ImageClickShow&flag="
                                + i + "&timestampFalg=" + random + "&verkey=LOGIN&" + r + "");
                get.setConfig(config);
                target = new HttpHost("new.hnair.com", 443, "https");
                get.setHeader("Referer", "http://new.hnair.com/hainanair/ibe/common/login.do");
                get.setHeader("Host", "new.hnair.com");
            ...
            String content = PropertiesUtils.getProperty("question.properties", map.get("key"));
            // ��ȡkey��Ӧ��valueֵ
            ...
            for (Integer b : bingo) {
                verificationCode.append(map.get(b + "")).append("|");
            }
    ...
    private String httpPostImg(String imgeName) throws Exception {
        // HttpGet get = new HttpGet("http://ocr.wdku.net/");
        // get.setConfig(config);

        int ran = ((int) (10 * Math.random())) % 5 + 1;
        long i = (long) (100000000000000l * ran * (Math.random()));
        URL u = null;
        HttpURLConnection con = null;
        String boundary = "-----------------------------1" + i + "";// +
        // MD5(String.valueOf(time));
        String boundarybytesString = boundary + "\r\n";
        OutputStream out = null;

        u = new URL("http://ocr.wdku.net/Upload");
        ...
        con.setRequestProperty("Content-Length", "1581");
        con.setRequestProperty("Cookie", "PHPSESSID=j7o76v7t5h4cgjgof62tnvrle4");
        con.setRequestProperty("Host", "ocr.wdku.net");
        con.setRequestProperty("Connection", "keep-alive");
        con.setRequestProperty("Pragma", "no-cache");
        con.setRequestProperty("Referer", "http://ocr.wdku.net/");
        ...
        // }
        out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
        String paramString = "Content-Disposition: form-data; name=\"id\"" + "\r\n\r\nWU_FILE_0\r\n";
        out.write(paramString.getBytes(StandardCharsets.UTF_8));

        out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
        paramString = "Content-Disposition: form-data; name=\"name\"" + "\r\n\r\n" + imgeName + ".jpg\r\n";
        out.write(paramString.getBytes(StandardCharsets.UTF_8));

        SimpleDateFormat sf = new SimpleDateFormat("EEE MMM dd yyyy HH:mm:ss", Locale.ENGLISH);
        String currentTime = sf.format(new Date()) + " GMT+0800";
        // System.out.println(sf.format(new Date()));

        out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
        paramString = "Content-Disposition: form-data; name=\"type\"" + "\r\n\r\nimage/jpeg\r\n";
        out.write(paramString.getBytes(StandardCharsets.UTF_8));

        out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
        paramString = "Content-Disposition: form-data; name=\"lastModifiedDate\"" + "\r\n\r\n" + currentTime + "\r\n";
        out.write(paramString.getBytes(StandardCharsets.UTF_8));

        // out.write(data);
        String fileUri = "E://testImg//" + imgeName + ".jpg";
        ...
        if (null != input) {
            ...
            if (data.length > 0) {
                 ...
                 */
                out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
                paramString = "Content-Disposition: form-data; name=\"size\"" + "\r\n\r\n" + file.length() + "\r\n";
                out.write(paramString.getBytes(StandardCharsets.UTF_8));

                out.write(boundarybytesString.getBytes(StandardCharsets.UTF_8));
                paramString = "Content-Disposition: form-data; name=\"file\"; filename=\"" + imgeName
                        + ".jpg\"\r\nContent-Type: image/jpeg\r\n\r\n";
                out.write(paramString.getBytes(StandardCharsets.UTF_8));

                out.write(data);
                // result =
                // YunSu.httpPostImage("http://api.ysdm.net/create.xml", param,
                // data);
            }
        }
    ...
    public String StartCreateOrder(String orderJson) {
        ...
        // String fltno = json.getString("flightNo");
        // String price = json.getString("price");
        // childrenUser = json.getString("username");
        // order_id = json.getString("id");
        // // String cookie =
        // // "JSESSIONID=DqB10YTJEvvdxS8W9qbOZwt M.mfb2cServer2;
        // // Webtrends=123.103.125.235.1521425259625401;
        // // Hm_lvt_a0f83bbc1d09b6e5a33928301f402485=1521425673;
        // //
        // WT_FPC=id=2b9903bf2c46434af221521425672604:lv=1521425708355:ss=1521425672604;
        // // SSO_CHECK=13532989542:20180412160308116;
        // // X-LB=1.3bd.46b.9078738d";
        // String cookie = "";
        // String back = "";
        // // ��½�˺�
        // cookie = login(orderJson, session);
        ...
        try {
            ...
            String fltno = json.getString("flightNo");
            String price = json.getString("price");
            // departure\":\"HAK\",\"arrival\":\"CGQ\",\"departureDate
            String depatureDate = json.getString("departureDate");
...
//			cookie = login(httpclient, config, cookieStore, cookie, orderJson, session);
            ...
            // �����ѯ����ҳ��
            receiveMsg(httpclient, config, cookieStore);
//			back = findFlights(httpclient, config, cookieStore, cookie, json);
        } catch (Exception e) {
    ...
    private void receiveMsg(CloseableHttpClient httpclient, RequestConfig config, BasicCookieStore cookieStore) {
        try {
//			HttpGet get = new HttpGet("http://api.fxhyd.cn/UserInterface.aspx?action=getmobile&token=01022378270da4019fa3d1f1d5399fcaaefb0cc66c01&itemid=51");
//			get.setConfig(config);
//			get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
//			get.setHeader("Host", "api.fxhyd.cn");
//			CloseableHttpResponse response = httpclient.execute(get);
//			String back = EntityUtils.toString(response.getEntity(), "utf-8");
//			System.out.println(back);
            HttpGet get = new HttpGet("www.tianjin-air.com");
            get.setConfig(config);
            get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
    ...
    private String findFlights(CloseableHttpClient httpclient, RequestConfig config, BasicCookieStore cookieStore,
                               String cookie, JSONObject json) {
        String dep = json.getString("departure");
        String arr = json.getString("arrival");
        String depdate = json.getString("departureDate");
        try {
//		HttpGet get = new HttpGet("http://new.hnair.com/hainanair/ibe/deeplink/ancillary.do?DD1=" + depdate
//				+ "&DD2=&TA=1&TC=0&TI=&TM=&TP=&ORI=" + dep + "&DES=" + arr
//				+ "&SC=Y&ICS=F&PT=F&FLC=1&NOR=&PACK=T&HC1=&HC2=&NA1=&NA2=&NA3=&NA4=&NA5=&NC1=&NC2=&NC3=&NC4=");
//		get.setConfig(config);
//		get.setHeader("Referer", "http://www.hnair.com/");
//		get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
//		get.setHeader("Host", "new.hnair.com");
//		get.setHeader("Cookie", cookie);
//
...
//		cookie = cookie + buf.toString();
            ...
            config = RequestConfig.custom().setSocketTimeout(timeout).setConnectTimeout(timeout)
                    .setConnectionRequestTimeout(timeout).setRedirectsEnabled(false).build();
            HttpHost target = new HttpHost("b2capi.donghaiair.cn", 80, "http");
            HttpPost post = new HttpPost("/ibe/flightBooking");
            post.setConfig(config);
            List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
            nameValue.add(new BasicNameValuePair("adult", "1"));
            nameValue.add(new BasicNameValuePair("child", "0"));
            nameValue.add(new BasicNameValuePair("departureDateStr", "2018-11-25"));
            nameValue.add(new BasicNameValuePair("destCode", "SYX"));
            nameValue.add(new BasicNameValuePair("flightType", "1"));
            nameValue.add(new BasicNameValuePair("infant", "0"));
            nameValue.add(new BasicNameValuePair("orgCode", "SZX"));
            nameValue.add(new BasicNameValuePair("returnDateStr", "2018-11-27"));
            String jsonObject = "{\"actionCode\":1,\"flights\":[{\"cabinType\":\"Y\",\"roundTrip\":\"OW\",\"flightNum\":\"DZ6265\",\"airtype\":\"738\",\"orgcity3code\":\"SZX\",\"destcity3code\":\"SYX\",\"stopcity3code\":\"\",\"stopairport3code\":\"\",\"departureDateStr\":\"2018-12-10\"}],\"orderPassengers\":[{\"personType\":\"ADULT\",\"personName\":\"����\",\"firstName\":\"��\",\"lastName\":\"��\",\"personIdtype\":\"NI\",\"personIdnum\":\"441423199501081014\",\"cabinType\":\"Y\",\"totalmoney\":640}],\"orderContact\":{\"name\":\"����\",\"firstName\":\"��\",\"lastName\":\"��\",\"phoneNum\":\"13532989542\",\"defaultContact\":\"\",\"email\":\"\"},\"channelType\":\"B2C\",\"resourceType\":\"PC\"}";
            post.setEntity(new StringEntity(jsonObject, StandardCharsets.UTF_8));
            post.setHeader("Referer",
                    "http://www.donghaiair.com/html/booking-manage/flight-condition.html?p=20181123165444");
            post.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
            post.setHeader("Host", "b2capi.donghaiair.cn");
            post.setHeader("Content-Type", "application/json;charset=utf-8");
            post.setHeader("b2c-api-user-token", "UM5u3alZTs3C2jIZ1149FnvXBBYEfcIXrpwRUTulC7H0agZRfkSeTg==");
...
//				"http://new.hnair.com/hainanair/ibe/deeplink/ancillary.do?DD1=" + depdate
//						+ "&DD2=&TA=1&TC=0&TI=&TM=&TP=&ORI=" + dep + "&DES=" + arr
//						+ "&SC=Y&ICS=F&PT=F&FLC=1&NOR=&PACK=T&HC1=&HC2=&NA1=&NA2=&NA3=&NA4=&NA5=&NC1=&NC2=&NC3=&NC4=");
//		get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
//		get.setHeader("Host", "new.hnair.com");
...
//				.setConnectionRequestTimeout(timeout).setRedirectsEnabled(false).build();
//		cookie = cookie + buf.toString();
//		post = new HttpPost("http://new.hnair.com/hainanair/ibe/common/processSearch.do");
//		post.setConfig(config);
//		post.setHeader("Referer",
//				"http://new.hnair.com/hainanair/ibe/common/processSearchEntry.do?fromEntryPoint=true");
//		post.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
//		post.setHeader("Host", "new.hnair.com");
...
//		get.setConfig(config);
//		get.setHeader("Referer", "http://new.hnair.com/hainanair/ibe/common/processSearchEntry.do?fromEntryPoint=true");
//		get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0");
//		get.setHeader("Host", "new.hnair.com");
...
//		back = EntityUtils.toString(response.getEntity(), "utf-8");
    ...
    private String toLogin(String orderJson, String session) {
        String content = "";
        String cookie = "";
        String location = "";
        String childrenUser = "";
        ...
        try {
            ...
            childrenUser = json.getString("username");
            order_id = json.getString("id");
            // String account = "13532989542_feeye123";
            ...
            buffer.append(
                    "<url>" + "https://login.xiamenair.com/cas/login?locale=zh_CN&service=https://www.xiamenair.com/zh-cn/Login/Login"
                            + "</url> ");
            buffer.append("<firstRequest>true</firstRequest>");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>get</method>");
            buffer.append("<type>5</type> ");
            buffer.append("<max>20</max> ");
            buffer.append("<hostIp>login.xiamenair.com</hostIp> ");
            buffer.append("<hostPart>443</hostPart> ");
            buffer.append("<encod>utf-8</encod> ");
            buffer.append("<proxyIp></proxyIp>");
            buffer.append("<proxyPort></proxyPort>"); // ʹ�ô���IPʱ��typeӦΪ3
            buffer.append("<headers>");
            buffer.append("<head name='Referer'>"
                    + "https://login.xiamenair.com/cas/login?locale=zh_CN&service=https://www.xiamenair.com/zh-cn/Login/Login"
                    + "</head >");
            buffer.append("<head name='Host'>login.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
            buffer.append("</headers>");
            buffer.append("</feeye-official>");

            content = OfficialMain.setRequestParams(buffer.toString());
            if (StringUtils.isNotEmpty(content) && content.contains("#@_@#")) {
                cookie = content.split("#@_@#")[0];
                content = content.split("#@_@#")[1];
            }
            Document doc = Jsoup.parse(content);
            Elements mb20 = doc.getElementsByClass("mb20");
            Element input_eventId = mb20.get(0).getElementsByAttributeValue("name", "_eventId").get(0);
            Element input_lt = mb20.get(0).getElementsByAttributeValue("name", "lt").get(0);
            Element input_execution = mb20.get(0).getElementsByAttributeValue("name", "execution").get(0);
            String _eventId = input_eventId.val();
            String lt = input_lt.val();
            String execution = input_execution.val();
            Element loginToken = doc.getElementById("loginToken");
            String loginTokenName = loginToken.attr("name");
            String loginTokenValue = loginToken.val();

            // ��ȡ��֤��
            String code = getcaptcha(session, cookie, "");
            ...
            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>/cas/login?locale=zh_CN&service=https://www.xiamenair.com/zh-cn/Login/Login</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            ...
            buffer.append("<param name='_eventId'>" + _eventId + "</param>");
            buffer.append("<param name='code'>" + code + "</param>");
            buffer.append("<param name='execution'>" + execution + "</param>");
            buffer.append("<param name='" + loginTokenName + "'>" + loginTokenValue + "</param>");
            buffer.append("<param name='lt'>" + lt + "</param>");
            buffer.append("<param name='msgCode'></param>");
            buffer.append("<param name='password'>" + userAccount[1] + "</param>");
            buffer.append("<param name='sbt1'>��¼</param>");
            buffer.append("<param name='sendcode'></param>");
            buffer.append("<param name='telephone'></param>");
            buffer.append("<param name='username'>" + userAccount[0] + "</param>");
            buffer.append("</params>");
            buffer.append("<headers>");
            ...
            // ��һ�ڶ����ض�������
            for (int i = 0; i < 2; i++) {
                if (map.get("location") != null && StringUtils.isNotEmpty(map.get("location"))) {
                    location = map.get("location");
                }
                if (map.get("cookie") != null && StringUtils.isNotEmpty(map.get("cookie"))) {
                    cookie = map.get("cookie");
                }
                map = is3wHostRedirct(location, session, cookie);
            }
            if (map == null || map.size() == 0) {
                sendCreateOrderInfo("error", "����ʧ��", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true");
                return "ERROR:����ʧ��";
            }
            location = map.get("location");
            cookie = map.get("cookie");
            if (StringUtils.isEmpty(location) || StringUtils.isEmpty(cookie)) {
                sendCreateOrderInfo("error", "����ʧ��", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true");
                return "ERROR:����ʧ��";
            }
            // �������ض�������
            map = isLoginHostRedirct(location, session, cookie);
            if (map == null || map.size() == 0) {
                sendCreateOrderInfo("error", "����ʧ��", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true");
                return "ERROR:����ʧ��";
    ...
    private Map<String, String> isLoginHostRedirct(String location, String session, String cookie) {
        Map<String, String> mapResult = new HashMap<String, String>();
        StringBuffer buffer = new StringBuffer();
        buffer.append("<feeye-official>");
        buffer.append("<official>" + "MF" + "</official> ");
        buffer.append("<url>" + location + "</url> ");
        buffer.append("<session>" + session + "</session>");
        buffer.append("<token>keep</token>");
        buffer.append("<method>get</method>");
        buffer.append("<redict>1</redict>");
        buffer.append("<firstRequest>true</firstRequest>");
        buffer.append("<protocol>http.protocol.allow-circular-redirects</protocol>");
        buffer.append("<type>5</type> ");
        buffer.append("<max>20</max> ");
        buffer.append("<hostIp>login.xiamenair.com</hostIp> ");
        buffer.append("<hostPart>443</hostPart> ");
        ...
        if (StringUtils.isNotEmpty(content) && content.contains("#@_@#")) {
            cookie = content.split("#@_@#")[0];
            String url = content.split("#@_@#")[2];
            mapResult.put("cookie", cookie);
            mapResult.put("location", url);
        }
        return mapResult;
    }

    private Map<String, String> is3wHostRedirct(String location, String session, String cookie) {
        Map<String, String> mapResult = new HashMap<String, String>();
        StringBuffer buffer = new StringBuffer();
        buffer.append("<feeye-official>");
        buffer.append("<official>" + "MF" + "</official> ");
        ...
        buffer.append("<type>5</type> ");
        buffer.append("<max>20</max> ");
        buffer.append("<hostIp>www.xiamenair.com</hostIp> ");
        buffer.append("<hostPart>443</hostPart> ");
        buffer.append("<encod>utf-8</encod> ");
        buffer.append("<proxyIp></proxyIp>");
        buffer.append("<proxyPort></proxyPort>"); // ʹ�ô���IPʱ��typeӦΪ3
        ...
        buffer.append(
                "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
        buffer.append("<head name='Host'>www.xiamenair.com</head>");
        buffer.append("</headers>");
        buffer.append("</feeye-official>");
        String content = OfficialMain.setRequestParams(buffer.toString());
        System.out.println(content);
        if (StringUtils.isNotEmpty(content) && content.contains("#@_@#")) {
            cookie = content.split("#@_@#")[0];
            Document doc = Jsoup.parse(content.split("#@_@#")[1]);
            Element ahref = doc.getElementsByTag("a").get(0);
    ...
    private String getcaptcha(String session, String cookie, String otheraccount) {
        ...
        try {
            ...
            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>" + "https://login.xiamenair.com/cas/captcha.htm?" + random + "" + "</url> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>get</method>");
            buffer.append("<type>5</type> ");
            ...
            buffer.append("<hostPart>443</hostPart> ");
            buffer.append("<encod>utf-8</encod> ");
            buffer.append("<codeUrl>true" + "@" + otheraccount + "</codeUrl>");
            ...
            buffer.append("<head name='Referer'>"
                    + "https://login.xiamenair.com/cas/login?locale=zh_CN&service=https://www.xiamenair.com/zh-cn/Login/Login"
                    + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
            buffer.append("</headers>");
    ...
    private String redictUrl(String location, String cookies, String session, String confirmRerer) {
        String content = "";
        try {
            StringBuffer buffer = new StringBuffer();
            ...
            buffer.append("<method>get</method>");
            buffer.append("<max>20</max> ");
            buffer.append("<hostIp>et.xiamenair.com</hostIp> ");
            buffer.append("<hostPart>443</hostPart> ");
            buffer.append("<encod>utf-8</encod> ");
            buffer.append("<proxyIp></proxyIp>");
            buffer.append("<proxyPort></proxyPort>"); // ʹ�ô���IPʱ��typeӦΪ3
            buffer.append("<headers>");
            buffer.append("<head name='Cookie'>" + cookies + "</head >");
            buffer.append("<head name='Referer'>" + confirmRerer + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
            ...
            content = OfficialMain.setRequestParams(buffer.toString());
            System.out.println("after redict request content:" + content);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return content;
    ...
    private String confirmAction(String token, String cookie, String session, String confirmRerer) {
        try {
            StringBuffer buffer = new StringBuffer();

            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>/xiamenair/book/confirm.action</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<firstRequest>true</firstRequest>");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            ...
            buffer.append("<proxyPort></proxyPort>"); // ʹ�ô���IPʱ��typeӦΪ3
            buffer.append("<params>");
            buffer.append("<param name='token'>" + token + "</param>");
            buffer.append("</params>");
            buffer.append("<headers>");
            buffer.append("<head name='Cookie'>" + cookie + "</head >");
            buffer.append("<head name='Referer'>" + confirmRerer + "</head >");
            ...
            String content = OfficialMain.setRequestParams(buffer.toString());
            System.out.println("after confirm request content:" + content);
            if (StringUtils.isNotEmpty(content) && content.contains("#@_@#")) {
                if (content.split("#@_@#").length == 3) {
                    String location = content.split("#@_@#")[2];
                    return location;
                }
            }
        } catch (Exception e) {
    ...
    private String viewAction(List<Map<String, String>> paramList, String cookie, String session) {
        String sureRerer = "https://et.xiamenair.com/xiamenair/book/input.action";
        String token = "";
        try {
            StringBuffer buffer = new StringBuffer();
            ...
            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>/xiamenair/book/view.action</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>post</method>");
            ...
            buffer.append("<proxyPort></proxyPort>"); // ʹ�ô���IPʱ��typeӦΪ3
            buffer.append("<params>");
            for (Map<String, String> parMap : paramList) {
                Iterator<Entry<String, String>> parIter = parMap.entrySet().iterator();
                while (parIter.hasNext()) {
                    Entry<String, String> par = parIter.next();
                    buffer.append("<param name='" + par.getKey() + "'>" + par.getValue() + "</param>");
                }
            }
            buffer.append("</params>");
            buffer.append("<headers>");
            buffer.append("<head name='Cookie'>" + cookie + "</head >");
            buffer.append("<head name='Referer'>" + sureRerer + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
            ...
            System.out.println(content);
            Document doc = Jsoup.parse(content);
            Elements tokenEle = doc.getElementsByAttributeValue("name", "token");
            token = tokenEle.val();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private String inputFlight(Map<String, String> paramMap, String cookie, String session) {
        String content = "";
        try {
            String referer = "http://et.xiamenair.com/xiamenair/book/findFlights.action?tripType=0"
                    + "&queryFlightInfo=" + paramMap.get("flights[0].departure") + ","
                    + paramMap.get("flights[0].arrival") + "," + paramMap.get("fqc.departureDateStr");
            Iterator<Entry<String, String>> paramIter = paramMap.entrySet().iterator();

            StringBuffer buffer = new StringBuffer();

            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>/xiamenair/book/input.action</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>post</method>");
            ...
            while (paramIter.hasNext()) {
                Entry<String, String> paramEntry = paramIter.next();
                buffer.append("<param name='" + paramEntry.getKey() + "'>" + paramEntry.getValue() + "</param>");
            }
            buffer.append("</params>");
            buffer.append("<headers>");
            buffer.append("<head name='Cookie'>" + cookie + "</head >");
            buffer.append("<head name='Referer'>" + referer + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
    ...
    private String queryFlightInfo(String orderJson, String cookies, String session) {
        String content = "";
        try {
            JSONObject json = new JSONObject(orderJson);
            String dep = json.getString("departure");
            ...
            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>https://et.xiamenair.com/xiamenair/book/findFlights.action?lang=zh&tripType=0"
                    + "&queryFlightInfo=" + dep + "," + arr + "," + depdate + "</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>get</method>");
            ...
            buffer.append("<headers>");
            buffer.append("<head name='Cookie'>" + cookies + "</head >");
            buffer.append("<head name='Referer'>" + "https://www.xiamenair.com/zh-cn/Home/Index" + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
            ...
            Document doc = Jsoup.parse(content);
            Elements element = doc.getElementsByAttributeValue("id", "random");
            String random = element.val();
            buffer = new StringBuffer();

            buffer.append("<feeye-official>");
            buffer.append("<official>" + "MF" + "</official> ");
            buffer.append("<url>https://et.xiamenair.com/xiamenair/book/zhaohban.json" + "?r=" + random
                    + "&lang=zh&takeoffDate=" + depdate + "&returnDate=&orgCity=" + dep + "" + "&dstCity=" + arr
                    + "&tripType=0&channelId=1" + "&_=" + new Date().getTime() + "</url> ");
            buffer.append("<type>5</type> ");
            buffer.append("<session>" + session + "</session>");
            buffer.append("<token>keep</token>");
            buffer.append("<method>get</method>");
            ...
            buffer.append("<head name='Cookie'>" + cookies + "</head >");
            buffer.append("<head name='Referer'>"
                    + "https://et.xiamenair.com/xiamenair/book/findFlights.action?lang=zh&tripType=0&queryFlightInfo="
                    + dep + "," + arr + "," + depdate + "" + "</head >");
            buffer.append("<head name='Host'>et.xiamenair.com</head>");
            buffer.append("<head name='Accept'>text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.8</head>");
            buffer.append(
                    "<head name='User-Agent'>Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0) Gecko/20100101 Firefox/43.0</head>");
    ...
    public static String sendCreateOrderInfo(String result, String message, String price, String childrenUser,
        ...
        try {
            // String orderUrl = PropertiesUtils.getPropertiesValue("config",
            // "orderUrl");
            String orderUrl = "";
            StringBuffer buffer = new StringBuffer();
            buffer.append("<feeye-official>");
