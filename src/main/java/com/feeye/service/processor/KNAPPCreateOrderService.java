        ...
        try {
            ...
            try {
                if (requestType == 1) {
                    ...
                    while (count > 0) {
                            ...
                            if (StringUtils.isEmpty(cookie)) {
                                ...
                                for (int loginIndex = 0; loginIndex < 5; loginIndex++) {
                                    ...
                                    if (cookie.contains("请填写正确的云速账号")) {
                                        sendCreateOrderInfo("error", "请填写正确的云速账号", "", childrenUser, newOrderNo,
                        ...
                        // 返回的结果格式为
                        // 乘机人1##证件号码##票号##BANKNO#_#乘机人2##证件号码##票号##BANKNO#_#@_@流水号
            ...
            for (; index < 3; index++) {
                ...
                if (resultMap == null || resultMap.size() == 0) {
                    if (retryCount < 9) {
                        ...
                        sendCreateOrderInfo("error", "查询不到航班", "", childrenUser, "", order_id, "", "", null, "", "",
                                "false", "true", billNo, "", requestType);
                        return "error:查询不到航班";
                    }
            ...
            if (index == 3) {
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "查询航班失败", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    return "error:查询航班失败";
                }
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    logger.error("error", e);
                    sendCreateOrderInfo("error", "解析航班数据异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
            ...
            if (verifyPostParam == null || verifyPostParam.size() == 0) {
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "无符合条件航班", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
            ...
            if (cookie.contains("用户或密码不正确")) {
                sendCreateOrderInfo("error", "用户或密码不正确", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                return "用户或密码不正确";
            }
            if (cookie.contains("密码错误")) {
                sendCreateOrderInfo("error", "请配置正确的验证码账号", "", childrenUser, "", order_id, "", "", null, "", "",
                        "false", "true", billNo, "", requestType);
                return "请配置正确的验证码账号";
            }
            if (cookie.contains("密码连续错误")) {
                sendCreateOrderInfo("error", "验证码账号或密码连续错误，请稍后再试", "", childrenUser, "", order_id, "", "", null, "", "",
                        "false", "true", billNo, "", requestType);
                return "验证码账号或密码连续错误，请稍后再试";
            }
            if (cookie.contains("请填写正确的云速账号")) {
                sendCreateOrderInfo("error", "请填写正确的云速账号", "", childrenUser, "", order_id, "", "", null, "", "",
                        "false", "true", billNo, "", requestType);
                return "请填写正确的云速账号";
            }
            if ("cancel".equals(cancel(cancelUrl, order_id, childrenUser))) {
            ...
            if (verifyPostParam.get("shoppingKey") == null || StringUtils.isEmpty(verifyPostParam.get("shoppingKey"))) {
                sendCreateOrderInfo("error", "未找到对应价格", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                return "未找到对应价格";
            }
            // 选择航班
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "选择航班异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    logger.error("error", e);
                    return "error:选择航班异常";
                }
            ...
            if (resultMap == null || resultMap.size() == 0) {
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "选择航班异常，重新创单", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    return "error:选择航班异常";
                }
            }
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "获取账号id异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    logger.error("error", e);
            ...
            if (StringUtils.isEmpty(memberId)) {
                sendCreateOrderInfo("error", "获取账号id异常", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                return "error:获取账号id异常";
            }
            // 新增乘客
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "添加乘客异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    logger.error("error", e);
                    return "error:添加乘客异常";
                }
            ...
            if (StringUtils.isNotEmpty(back)
                    && (back.contains("404错误") || back.contains("出错啦") || back.contains("您要查看的页面不存在或已删除"))) {
                if (retryCount < 9) {
                    sendOrderStatus(childrenUser, order_id, "返回结果异常，正在重试创单");
                    return StartCreateOrder(orderJson, ++retryCount, requestType);
                } else {
            ...
            if (passengerId.size() != passengers.length()) {
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "提交乘客数量有误", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "获取增值服务异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    logger.error("error", e);
            ...
            if (resultMap == null || resultMap.size() == 0) {
                sendCreateOrderInfo("error", "获取增值服务失败", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
            ...
            try {
                ...
                if (retryCount < 9) {
                    ...
                    sendCreateOrderInfo("error", "获取联系人Id异常", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    logger.error("error", e);
            ...
            if (StringUtils.isEmpty(back)) {
                sendCreateOrderInfo("error", "获取联系人Id失败", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
            ...
            if (back.contains("获取不到联系人id")) {
                ...
                try {
                    ...
                    if (retryCount < 9) {
                        ...
                        sendCreateOrderInfo("error", "获取默认联系人Id异常", "", childrenUser, "", order_id, "", "", null, "",
                                "", "false", "true", billNo, "", requestType);
                        return "error:获取联系人Id异常";
                    }
                }
                if (map == null || map.size() == 0) {
                    sendCreateOrderInfo("error", "获取默认联系人Id失败", "", childrenUser, "", order_id, "", "", null, "", "",
                            "false", "true", billNo, "", requestType);
                    return "error:获取联系人Id失败";
                } else {
            ...
            try {
                ...
                sendCreateOrderInfo("error", "提交订单信息异常", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                logger.error("error", e);
                return "error:提交订单信息异常";
            }
            if (StringUtils.isEmpty(back)) {
                sendCreateOrderInfo("error", "提交订单信息失败", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                return "error:提交订单信息失败";
            }
            ...
            try {
                ...
                sendCreateOrderInfo("error", "选择支付方式异常", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
                logger.error("error", e);
                return "error:选择支付方式异常";
            }
            if (resultMap == null || resultMap.size() == 0) {
                sendCreateOrderInfo("error", "选择支付方式失败", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
...
//				sendCreateOrderInfo("error", "创单超时，停止创单", "", childrenUser, "", order_id, "", "", null, "", "", "false",
//						"true", billNo, "", requestType);
//				return "创单超时，停止创单";
//			}
            /*
            ...
            try {
                ...
                logger.error("createOrderResultError" + e);
                sendCreateOrderInfo("error", "请求生成订单异常，请到官网确认是否已经生成订单", "", childrenUser, "", order_id, "", "", null,
                        "", "", "false", "true", billNo, "", requestType);
                return "error:请求生成订单异常，请到官网确认是否已经生成订单";
            }
            if (StringUtils.isEmpty(back)) {
                sendCreateOrderInfo("error", "请求生成订单未返回结果，请到官网确认是否已经生成订单", "", childrenUser, "", order_id, "", "", null,
                        "", "", "false", "true", billNo, "", requestType);
            ...
            if (url == null || StringUtils.isEmpty(url)) {
                ...
                sendCreateOrderInfo("error", "未获取到支付请求", "", childrenUser, "", order_id, "", "", null, "", "", "false",
                        "true", billNo, "", requestType);
...
//					if (resultMap == null || resultMap.size() == 0) {
//						sendCreateOrderInfo("error", "支付失败", "", childrenUser, transactionNumber, order_id, "", "",
...
//				}
//			} else {
//				sendCreateOrderInfo("error", "未获取到平台订单号", "", childrenUser, "", order_id, "", "", null, "", "", "false",
//						"true", billNo, "", requestType);
//			}
            if (resultMap == null || resultMap.size() == 0) {
                sendCreateOrderInfo("error", "支付异常，请到官网检查是否已经支付", "", childrenUser, transactionNumber, order_id, "", "",
                        null, "", "", "false", "true", billNo, "", requestType);
            ...
            if (StringUtils.isEmpty(bankNo)) {
                sendCreateOrderInfo("error", "支付未知错误，请到官网确认是否已支付", "", childrenUser, transactionNumber, order_id, "",
                        "", null, "", "", "false", "true", billNo, "", requestType);
                ...
                try {
                    ...
                    if (StringUtils.isEmpty(cookie)) {
                        ...
                        for (int loginIndex = 0; loginIndex < 5; loginIndex++) {
                            ...
                            if (cookie.contains("请填写正确的云速账号")) {
                                sendCreateOrderInfo("error", "请填写正确的云速账号", "", childrenUser, newOrderNo, order_id, "",
                                        "", null, "", "", "false", "true", billNo, "", requestType);
                                return "SUCCESS#@@#请填写正确的云速账号";
                            }
                            Thread.sleep(500);
                    ...
                    // sendCreateOrderInfo("error", "获取票号信息异常", "",
                    ...
                    if (ticketnos.length == 2) {
                        ...
                        if (ticketCount.length == passengers.length() && !ticketnos[0].contains("null")) {
                            sendCreateOrderInfo("success", "", money, childrenUser, orderNo, order_id, "", "true",
                                    ticketnos[0], "易宝", ticketnos[1], "false", "true", billNo,
            ...
            logger.error(e);
            sendCreateOrderInfo("error", "创单异常", "", childrenUser, "", order_id, "", "", null, "", "", "false", "true",
                    billNo, "", requestType);
            return "ERROR:创单异常";
        } finally {
            try {
        ...
        for (int i = 0; i < passengerId.size(); i++) {
            orderConfirmPostParam.append(
                    "{\"id\":" + passengerId.get(i) + ",\"ancillaryGroups\":[{\"ancillaries\":[],\"flightNo\":\""
                            ...
                            + orgAirportName + "\"},\"transpart\":\"1\",\"tripTypeString\":\"去程\"}]},");
        }
     ...
     * 格式为:姓名##生份证##票号 String payTransactionid =
