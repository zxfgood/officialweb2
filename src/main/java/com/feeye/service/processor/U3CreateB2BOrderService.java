...
public class U3CreateB2BOrderService {
	private static final Logger logger = Logger.getLogger(U3CreateB2BOrderService.class);
	private static final int timeout = 60000;
	private static Map<String, String> account3Ub2b = new HashMap<String, String>();
...
//				null,SSLConnectionSocketFactory.getDefaultHostnameVerifier());
		...
		try {
			...
			if ("cancel".equals(cancel(cancelUrl, order_id, childrenUser))) {
				// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "已取消出票" );
				sendCreateOrderInfo("error", "已取消出票", "", childrenUser, "", order_id, "", "", null, "", "", "false",
						"true", billNo, requestType, ticketPrice);
				logger.info(order_id + "已取消出票");
				return "取消出票";
			}
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "检查登录状态" );
			cookie = account3Ub2b.get(key);
			if (StringUtils.isEmpty(cookie)) {
				// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "开始登录" );
			...
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "PNR查询" );
			...
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "PNR入库" );
			...
			if (PnrHostinParamMap == null) {
				...
				// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "查询订单" );
			...
			// 进行支付
			String orderInfo = "";
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "开始支付" );
			for (int i = 0; i < 3; i++) {
				if ("cancel".equals(cancel(cancelUrl, order_id, childrenUser))) {
					// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "已取消出票" );
					sendCreateOrderInfo("error", "已取消出票", "", childrenUser, orderNo, order_id, "", "", null, "", "",
				...
				if (StringUtils.isEmpty(orderInfo)) {
					...
					if (tdHtml.contains("出票完成")) {
						...
						// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "手动出票" );
						frontOrderQueryMap = frontOrderQuery(httpclient, config, cookie, orderJson, context);
						result = frontOrderDetailQuery(httpclient, config, cookie, orderJson, frontOrderQueryMap, context);
						...
						// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "重新支付" );
						orderInfo = virtualPay(httpclient, config, cookie, orderJson, PnrHostinParamMap, context);
						if (StringUtils.isEmpty(orderInfo)) {
							continue;
						}
			...
			sendCreateOrderInfo("success", "", "", childrenUser, orderNo, order_id, "true", "", null, "预付款", "",
					"false", "true", billNo, requestType, ticketPrice);
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "获取票号" );
			if(StringUtils.isEmpty(orderInfo)){
				Map<String, String> frontOrderQueryMap = frontOrderQuery(httpclient, config, cookie, orderJson, context);
				...
				try {
					if (StringUtils.isNotEmpty(orderInfo)) {
						...
						if (ticketnos.length == 2) {
							// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "回填票号" );
	...
	String parseDetail(String detailresult, String orderJson, String orderNo) {
		try {
			StringBuilder sbu = new StringBuilder();
			JSONObject json = new JSONObject(orderJson);
			String bigpnr = json.getString("bigpnr");
	...
	private String orderTicketOut(CloseableHttpClient httpclient, RequestConfig config, String cookie, String orderJson,
		...
		try {
			HttpPost post = new HttpPost("/3ub2b/OrderTicketOut.do");
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			...
			post.setHeader("Host", "3u.travelsky.com.cn");
			post.setHeader("Cookie", cookie);
			post.setHeader("Origin", "https://3u.travelsky.com.cn");
			post.setHeader("Referer", "https://3u.travelsky.com.cn/3ub2b/PnrHostin.do");
	...
	private String frontOrderDetailQuery(CloseableHttpClient httpclient, RequestConfig config, String cookie,
		...
		CloseableHttpResponse response = null;
		try {
			HttpPost post = new HttpPost("/3ub2b/FrontOrderDetailQuery.do");
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
			...
			post.setHeader("Cookie", cookie);
			post.setHeader("Origin", "https://3u.travelsky.com.cn");
			post.setHeader("Referer", "https://3u.travelsky.com.cn/3ub2b/FrontOrderQuery.do");
			post.setHeader("Proxy-Authorization" , "Basic " + Base64.encodeBase64String(dlyAccount.getBytes("utf-8")));
			post.setHeader("Proxy-Connection", "keep-alive");
			post.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0)	 Gecko/20100101 Firefox/43.0");
			response = httpclient.execute(target, post, context);
...
//			logger.info("查询订单详情返回:" + back);
	...
	private Map<String, String> frontOrderQuery(CloseableHttpClient httpclient, RequestConfig config, String cookie,
		...
		try {
			JSONObject json = new JSONObject(orderJson);
			String bigPnr = json.getString("bigpnr");
			HttpPost post = new HttpPost("/3ub2b/FrontOrderQuery.do");
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
...
//			logger.info("查询返回:" + back);
	...
	private String virtualPay(CloseableHttpClient httpclient, RequestConfig config, String cookie, String orderJson,
		...
		try {
			JSONObject json = new JSONObject(orderJson);
			JSONArray passengers = json.getJSONArray("passengers");
			HttpPost post = new HttpPost("/3ub2b/VirtualPay.do");
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
			for (Map.Entry<String, String> entry : paramMap.entrySet()) {
				nameValue.add(new BasicNameValuePair(entry.getKey(), entry.getValue()));
	...
	private Map<String, String> PnrHostin(CloseableHttpClient httpclient, RequestConfig config, String cookie,
		...
		try {
			...
			HttpPost post = new HttpPost("/3ub2b/PnrHostin.do");
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
			for (Map.Entry<String, String> entry : paramMap.entrySet()) {
				nameValue.add(new BasicNameValuePair(entry.getKey(), entry.getValue()));
			...
			post.setHeader("Host", "3u.travelsky.com.cn");
			post.setHeader("Cookie", cookie);
			post.setHeader("Referer", "https://3u.travelsky.com.cn/3ub2b/PNRQuery.do");
			...
			if (StringUtils.isNotEmpty(randomNum)) {
				post = new HttpPost("/3ub2b/PnrHostin.do");
	...
	private Map<String, String> PNRQuery(CloseableHttpClient httpclient, RequestConfig config, String cookie,
		...
		try {
			...
			HttpGet get = new HttpGet("/3ub2b/PnrImportIndex.do");
			get.setConfig(config);
			HttpHost target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			get.setHeader("Host", "3u.travelsky.com.cn");
...
//			logger.info("首页返回：" + back);
			Thread.sleep(2000);
			response.close();
			HttpPost post = new HttpPost("/3ub2b/PNRQuery.do");
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
			nameValue.add(new BasicNameValuePair("BPnrNo", bigPnr));
			...
			post.setHeader("Host", "3u.travelsky.com.cn");
			post.setHeader("Cookie", cookie);
			post.setHeader("Referer", "https://3u.travelsky.com.cn/3ub2b/PnrImportIndex.do");
			post.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0)	 Gecko/20100101 Firefox/43.0");
			post.setHeader("Proxy-Authorization", "Basic " + Base64.encodeBase64String(dlyAccount.getBytes("utf-8")));
			post.setHeader("Proxy-Connection", "keep-alive");
			response = httpclient.execute(target, post,context);
...
//			logger.info("pnr查询返回：" + back);
			...
			response.close();
			post = new HttpPost("/3ub2b/DomesticPnrPriceCompute.do");
			nameValue = new ArrayList<NameValuePair>();
	...
	public String login(CloseableHttpClient httpclient, RequestConfig config, BasicCookieStore cookieStore,
		...
		try {
			...
			childrenUser = json.getString("username");
			get = new HttpGet(
					"/3ub2b/cas/authenticate.do?redirectUrl=https://3u.travelsky.com.cn/3ub2b/ssoClientLogin.do");
			get.setConfig(config);
			target = new HttpHost("3u.travelsky.com.cn", 443, "https");
			get.setHeader("Host", "3u.travelsky.com.cn");
			get.setHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; WOW64; rv:43.0)	 Gecko/20100101 Firefox/43.0");
			get.setHeader("Proxy-Authorization" , "Basic " + Base64.encodeBase64String(dlyAccount.getBytes("utf-8")));
			get.setHeader("Proxy-Connection", "keep-alive");
			...
			cookie = buf.toString();
			// PushOrderInfoUtil.sendOrderStatus( "3UB2B" , mainUser , childrenUser , order_id , "识别验证码" );
			// 获取验证码请求
			response.close();
			get = new HttpGet("/3ub2b/VerificationCode.do?agentType=domestic&0.5855130690663959");
			get.setConfig(config);
			get.setHeader("Host", "3u.travelsky.com.cn");
			get.setHeader("Cookie", cookie);
			get.setHeader("Proxy-Authorization" , "Basic " + Base64.encodeBase64String(dlyAccount.getBytes("utf-8")));
...
//			}
			...
			response.close();
			post = new HttpPost("/3ub2b/cas/login.do");
			post.setConfig(config);
			List<NameValuePair> nameValue = new ArrayList<NameValuePair>();
			...
			post.setHeader("Referer",
					"https://3u.travelsky.com.cn/3ub2b/cas/authenticate.do?redirectUrl=https://3u.travelsky.com.cn/3ub2b/ssoClientLogin.do");
			post.setHeader("Content-Type", "application/x-www-form-urlencoded");
			post.setHeader("Origin", "https://3u.travelsky.com.cn");
			...
			if (!newCookie.contains("TUB2BCOOKIE")) {
				return null;
			}
			cookie += newCookie;
	...
	public static String sendOrderStatus(String childrenUser, String orderId, String status) {
		try {
			...
			buffer.append("<official>3Ub2b</official> ");
